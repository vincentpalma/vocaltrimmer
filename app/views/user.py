...
def charge():
    try:
        ...
        user = models.User.query.filter_by(email=current_user.email).first()
        credits_to_add = int(str(request.json['description']).split(' ')[0])
        user.balance += credits_to_add
        db.session.commit()
        if credits_to_add == 1:
            flash(f'Payment Successful! {str(credits_to_add)} song has been added to your balance.', 'positive')
        else:
            flash(f'Payment Successful! {str(credits_to_add)} songs have been added to your balance.', 'positive')
        return jsonify({'status': 'success!'})
    except stripe.error.StripeError:
        flash('Something went wrong, please try again.', 'negative')
